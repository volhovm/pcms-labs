...
public class Helpers {
    ...
    public static String genScoped(String term, List<FunctionalParser.FuncContext> sub) {
        System.err.println("Term is: " + term);
        String ret = "";
    ...
    public static String genListConstr(List<FunctionalParser.TermContext> values) {
        return "[" + values.stream().map(a -> a.res).collect(Collectors.joining(",")) + "]";
    }
    public static String genApplication(String a, List<FunctionalParser.TermContext> values) {
        return a + "(" + (values.stream().map(x -> x.res).collect(Collectors.joining(","))) + ")";
    }
}
